...
ENGINE_MARK="${ENGINE_BUILD_MARK:-sc313-jackFork}"
...
grep -E 'jackFork|jackExternal|SC_AUDIO|scsynth connected|listening on OSC|Engine synths|ENGINE_TEST|ERROR|Boolean|syntax|JackTemporary|not understood|jackd ready|scsynth ready|WARN:' "$LOG" || true

if grep -qE 'ERROR:|MustBeBoolean|syntax error|not understood|Command line parse failed' "$LOG"; then
