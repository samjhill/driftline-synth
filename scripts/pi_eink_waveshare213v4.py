...
def _mark_display_ok() -> None:
    try:
        sys.path.insert(0, str(ROOT / "src"))
        from eink_status import write_eink_status

        write_eink_status("OK", patch_summary="panel refresh")
    except Exception:
        pass
...
def _check_spi_setup() -> None:
    ...
    if errors:
        ...
        print(f"pi-eink: {error_msg}", file=sys.stderr)
...
def main() -> int:
    ...
    # Test mode: if PI_EINK_TEST_MODE is set, just process images without hardware
    test_mode = _env_bool("PI_EINK_TEST_MODE", False)
    test_output = os.environ.get("PI_EINK_TEST_OUTPUT")
    
    if not _env_bool("PI_EINK_ENABLE", False) and not test_mode:
        # Exit cleanly when not enabled.
        return 0

    status_png = Path(os.environ.get("PI_EINK_STATUS_IMAGE_PATH", "/var/lib/pi-ambient-synth/eink-status.png"))
    refresh_seconds = float(os.environ.get("PI_EINK_REFRESH_SECONDS", "2"))

    # Waveshare 2.13" V4 (B/W) is 250x122
    epd_w = int(os.environ.get("PI_EINK_WIDTH", "250"))
    epd_h = int(os.environ.get("PI_EINK_HEIGHT", "122"))

    # In test mode, skip hardware initialization
    if test_mode:
        print("pi-eink: TEST MODE - processing images without hardware", file=sys.stderr)
        epd = None
    else:
        ...
        try:
            ...
            print(f"pi-eink: failed to import Waveshare EPD driver: {e}", file=sys.stderr)
            return 2

    try:
        # Init and clear (skip in test mode)
        if not test_mode:
            print("pi-eink: initializing display...", file=sys.stderr)
            try:
                epd.init()
            except TypeError:
                try:
                ...
                if e.errno == 9:  # Bad file descriptor
                    print("pi-eink: SPI communication error (errno 9: bad file descriptor)", file=sys.stderr)
                    print("pi-eink: This usually means SPI is not enabled or accessible.", file=sys.stderr)
                    print("pi-eink: Enable SPI: sudo raspi-config -> Interface Options -> SPI -> Enable", file=sys.stderr)
                    print("pi-eink: Then reboot: sudo reboot", file=sys.stderr)
                    return 3
                raise
            except Exception as e:
                print(f"pi-eink: init error: {e}", file=sys.stderr)
                import traceback
        ...
        if test_mode:
            ...
            print(f"pi-eink: processing {status_png} once (test mode)", file=sys.stderr)
            try:
                print(f"pi-eink: updating display...", file=sys.stderr)
                with Image.open(status_png) as im:
                    # Log original image info for debugging
                    print(f"pi-eink: source image: {im.size}, mode: {im.mode}", file=sys.stderr)
                    ...
                    if im.mode == "1":
                        ...
                        print(f"pi-eink: source has {src_black} black pixels ({src_black_pct:.1f}%)", file=sys.stderr)
                        if src_black == 0:
                            print("pi-eink: WARNING: source image is all white! GhostRoll may not be generating status correctly.", file=sys.stderr)
                    
                    frame = _fit_for_epd(im, w=epd_w, h=epd_h)
                    
                    # Log processed image info
                    print(f"pi-eink: processed image: {frame.size}, mode: {frame.mode}", file=sys.stderr)
                    ...
                    if frame.mode == "1":
                        ...
                        print(f"pi-eink: pixel stats: {black_count} black ({black_pct:.1f}%), {white_count} white (of {total} total)", file=sys.stderr)
                        if black_count == 0:
                            print("pi-eink: WARNING: processed image is all white! Text may have been lost during resize.", file=sys.stderr)
                        elif black_count < total * 0.01:  # Less than 1% black
                            print(f"pi-eink: WARNING: very few black pixels ({black_pct:.1f}%), text may not be visible", file=sys.stderr)
                        elif black_pct > 50:
                            print(f"pi-eink: NOTE: image is mostly black ({black_pct:.1f}%), may need inversion", file=sys.stderr)
                    ...
                    if test_output:
                        ...
                        print(f"pi-eink: saved processed image to {output_path}", file=sys.stderr)
                    else:
                        ...
                        print(f"pi-eink: saved processed image to {test_output_path}", file=sys.stderr)
                    print("pi-eink: display updated", file=sys.stderr)
                    _mark_display_ok()
            except FileNotFoundError:
                print("pi-eink: ERROR: status.png not found", file=sys.stderr)
                return 1
            except Exception as e:
                print(f"pi-eink: render error: {e}", file=sys.stderr)
                import traceback
                traceback.print_exc(file=sys.stderr)
                return 1
        ...
        if not test_mode:
            ...
            print(f"pi-eink: watching {status_png} (refresh: {refresh_seconds}s)", file=sys.stderr)
            last_mtime = 0.0
                        ...
                        with Image.open(status_png) as im:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    print(f"pi-eink: display method error: {e}", file=sys.stderr)
                                    import traceback
                                    traceback.print_exc(file=sys.stderr)
                                    raise
                        print("pi-eink: display updated", file=sys.stderr)
        ...
        print("pi-eink: shutting down...", file=sys.stderr)
